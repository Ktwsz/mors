...
def disjunctive_44(s, d):
    model.Add(assert_(index_sets_agree(s, d), 'disjunctive: the array arguments must have identical index sets', disjunctive_strict_45(s, d) if lb_array(d) > 0 else fzn_disjunctive_reif(s, d)) == True)
...
duration = Array(OBJECTS, Array([60, 45, 30, 30, 20, 15, 15, 15]))
handlers = Array(OBJECTS, Array([3, 2, 2, 1, 2, 1, 1, 2]))
trolleys = Array(OBJECTS, Array([2, 1, 2, 2, 2, 0, 0, 1]))
available_handlers = 4
available_trolleys = 3
available_time = 180
start = IntVarArray('start', OBJECTS, range(0, available_time + 1))
...
solution_printer = SolutionPrinter(start, end)
